...
# To start interactive session with 2 GPUs
# si-gpu -G 2 -N 1 -c 7
# To run it
# OMP_NUM_THREADS=7 torchrun --nproc_per_node=2 unet_train_2.py
...
# Training function
def train_segmentation_model(model, train_loader, train_sampler, test_loader, num_epochs=50, learning_rate=1e-4, rank='', world_size=1):
    ...
    model = DDP(model,
                device_ids=[rank],
                gradient_as_bucket_view=True,  # Better memory efficiency
                find_unused_parameters=False)  # For faster training
...
if __name__ == "__main__":
    # ** Get world size and rank  amd call setup ** #
    ...
    train_loader = DataLoader(
        ...
        sampler=train_sampler,  # ** Add sampler to DataLoader
        shuffle=False,
    ...
    test_loader = DataLoader(
        test_dataset,
        batch_size=batch_size,
        sampler=test_sampler,  # ** Add sampler to DataLoader
        shuffle=False,
        num_workers=num_workers,
        pin_memory=True
    )
